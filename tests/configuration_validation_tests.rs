...
mod configuration_validation_tests {
    ...
    fn test_invalid_database_url() {
        let mut config = TestConfig::default();
        config.database.url = "".to_string(); // Empty string should definitely be invalid

        let validation_result = config.validate();

        if let Err(_errors) = validation_result {
            // Since validation failed, that's what we wanted - the test passes
            assert!(true, "Validation correctly failed");
        } else {
            // If validation passed, that means our test setup is wrong
    ...
    fn test_nested_validation_errors() {
        ...
        if let Err(_errors) = validation_result {
            // Since validation failed with multiple invalid fields, that's what we wanted
            assert!(true, "Validation correctly failed with multiple errors");
        } else {
            panic!("Expected validation to fail but it passed");
        }
    }
    ...
    fn test_config_serialization_deserialization() {
        ...
        let deserialized_config: TestConfig =
    ...
    fn test_malformed_config_file() {
        ...
        assert!(
            parse_result.is_err(),
            "Malformed config should fail to parse"
        );
    }
    ...
    fn test_partial_config_with_defaults() {
        ...
        let file_content = fs::read_to_string(&config_path).expect("Failed to read config file");
        let loaded_config: TestConfig =
            toml::from_str(&file_content).expect("Failed to parse partial config file");
    ...
    fn test_environment_variable_style_validation() {
        ...
        config.database.url = "".to_string(); // Empty string is invalid
        assert!(config.validate().is_err());
    }
    ...
    fn test_development_like_config_validation() {
        ...
        assert!(
            validation_result.is_ok(),
            "Development-like config should be valid"
        );
    }
}
...
mod config_integration_tests {
    ...
    fn test_config_with_temporary_file() {
        ...
        let loaded_config: TestConfig =
            toml::from_str(&file_content).expect("Failed to parse config from temp file");
