...
mod crate_boundary_tests {
    ...
    fn test_user_crate_has_no_web_dependencies() {
        ...
        assert!(
            !user_cargo_toml.contains("axum"),
            "User crate should not depend on axum"
        );
        assert!(
            !user_cargo_toml.contains("askama"),
            "User crate should not depend on askama"
        );
        assert!(
            !user_cargo_toml.contains("tower"),
            "User crate should not depend on tower-http"
        );
        assert!(
            !user_cargo_toml.contains("imkitchen-web"),
            "User crate should not depend on web crate"
        );

        // User crate SHOULD only depend on shared types and external libraries
        assert!(
            user_cargo_toml.contains("imkitchen-shared"),
            "User crate should depend on shared types"
        );
        assert!(
            user_cargo_toml.contains("validator"),
            "User crate should have validation"
        );
        assert!(
            user_cargo_toml.contains("sqlx"),
            "User crate should have database access"
        );
    }
    ...
    fn test_web_crate_can_depend_on_user_crate() {
        ...
        assert!(
            web_cargo_toml.contains("imkitchen-user"),
            "Web crate should depend on user crate"
        );
        assert!(
            web_cargo_toml.contains("imkitchen-shared"),
            "Web crate should depend on shared types"
        );

        // Web crate SHOULD have web-specific dependencies
        assert!(
            web_cargo_toml.contains("axum"),
            "Web crate should use axum framework"
        );
        assert!(
            web_cargo_toml.contains("askama"),
            "Web crate should use askama templates"
        );
        assert!(
            web_cargo_toml.contains("tower-http"),
            "Web crate should use tower-http"
        );
    }
    ...
    fn test_shared_crate_has_minimal_dependencies() {
        ...
        assert!(
            !shared_cargo_toml.contains("imkitchen-user"),
            "Shared crate should not depend on user crate"
        );
        assert!(
            !shared_cargo_toml.contains("imkitchen-web"),
            "Shared crate should not depend on web crate"
        );

        // Shared crate should NOT have framework-specific dependencies
        assert!(
            !shared_cargo_toml.contains("axum"),
            "Shared crate should not depend on axum"
        );
        assert!(
            !shared_cargo_toml.contains("askama"),
            "Shared crate should not depend on askama"
        );

        // Shared crate SHOULD have only basic type dependencies
        assert!(
            shared_cargo_toml.contains("serde"),
            "Shared crate should have serialization"
        );
        assert!(
            shared_cargo_toml.contains("validator"),
            "Shared crate should have validation"
        );
        assert!(
            shared_cargo_toml.contains("uuid"),
            "Shared crate should have UUID support"
        );
    }
    ...
    fn test_domain_models_are_in_user_crate() {
        ...
        assert!(
            Path::new("crates/imkitchen-user/src/domain").exists(),
            "User domain logic should be in user crate"
        );
        assert!(
            Path::new("crates/imkitchen-user/src/domain/user.rs").exists(),
            "User aggregate should be in user crate"
        );
        assert!(
            Path::new("crates/imkitchen-user/src/events").exists(),
            "User events should be in user crate"
        );
        assert!(
            Path::new("crates/imkitchen-user/src/commands").exists(),
            "User commands should be in user crate"
        );
        assert!(
            Path::new("crates/imkitchen-user/src/queries").exists(),
    ...
    fn test_presentation_logic_is_in_web_crate() {
        ...
        assert!(
            Path::new("crates/imkitchen-web/src/handlers").exists(),
            "HTTP handlers should be in web crate"
        );
        assert!(
            Path::new("crates/imkitchen-web/templates").exists(),
            "HTML templates should be in web crate"
        );
        assert!(
            Path::new("crates/imkitchen-web/static").exists(),
            "Static assets should be in web crate"
        );

        // Check specific template files
        assert!(
            Path::new("crates/imkitchen-web/templates/auth/login.html").exists(),
            "Login template should be in web crate"
        );
        assert!(
            Path::new("crates/imkitchen-web/templates/dashboard/user.html").exists(),
    ...
    fn test_shared_types_are_in_shared_crate() {
        ...
        assert!(
            Path::new("crates/imkitchen-shared/src/types").exists(),
        ...
        assert!(
            types_mod.contains("pub struct Email"),
            "Email should be a shared type"
        );
        assert!(
            types_mod.contains("pub struct FamilySize"),
            "FamilySize should be a shared type"
        );
        assert!(
            types_mod.contains("pub enum SkillLevel"),
            "SkillLevel should be a shared type"
        );
    }
    ...
    fn test_no_circular_dependencies() {
        ...
        let user_deps = std::fs::read_to_string("crates/imkitchen-user/Cargo.toml")
            .expect("Failed to read user Cargo.toml");
        assert!(
            !user_deps.contains("imkitchen-web"),
            "No circular dependency: user -> web"
        );
        ...
        let shared_deps = std::fs::read_to_string("crates/imkitchen-shared/Cargo.toml")
            .expect("Failed to read shared Cargo.toml");
        assert!(
            !shared_deps.contains("imkitchen-user"),
            "No circular dependency: shared -> user"
        );
        assert!(
            !shared_deps.contains("imkitchen-web"),
            "No circular dependency: shared -> web"
        );
    }
    ...
    fn test_proper_dependency_direction() {
        ...
        assert!(
            cli_deps.contains("imkitchen-web"),
            "CLI should depend on web crate"
        );
        ...
        let web_deps = std::fs::read_to_string("crates/imkitchen-web/Cargo.toml")
            .expect("Failed to read web Cargo.toml");
        assert!(
            web_deps.contains("imkitchen-user"),
            "Web should depend on user crate"
        );
        assert!(
            web_deps.contains("imkitchen-shared"),
            "Web should depend on shared crate"
        );
        ...
        let user_deps = std::fs::read_to_string("crates/imkitchen-user/Cargo.toml")
            .expect("Failed to read user Cargo.toml");
        assert!(
            user_deps.contains("imkitchen-shared"),
            "User should depend on shared crate"
        );
        assert!(
            !user_deps.contains("imkitchen-web"),
            "User should NOT depend on web crate"
        );
    }
    ...
    fn test_domain_logic_purity() {
        ...
        assert!(
            !user_domain.contains("axum"),
            "Domain should not import axum"
        );
        assert!(
            !user_domain.contains("askama"),
            "Domain should not import askama"
        );
        assert!(
            !user_domain.contains("Html"),
            "Domain should not import HTML types"
        );
        assert!(
            !user_domain.contains("Response"),
            "Domain should not import HTTP response types"
        );

        // Domain SHOULD import domain-appropriate types
        assert!(
            user_domain.contains("Email") || user_domain.contains("imkitchen_shared"),
    ...
    fn test_presentation_layer_separation() {
        ...
        assert!(
            auth_handlers.contains("imkitchen_user") || auth_handlers.contains("Email"),
            "Presentation should use domain types"
        );
        assert!(
            auth_handlers.contains("DirectLoginService") || auth_handlers.contains("LoginCommand"),
            "Presentation should use domain services"
        );

        // Presentation SHOULD have web-specific imports
        assert!(
            auth_handlers.contains("axum"),
            "Presentation should use web framework"
        );
        assert!(
            auth_handlers.contains("Template") || auth_handlers.contains("askama"),
    ...
    fn test_event_sourcing_isolation() {
        ...
        assert!(
            !auth_handlers.contains("UserRegistered"),
            "Presentation should not directly handle events"
        );
        assert!(
            !auth_handlers.contains("UserLoggedIn"),
            "Presentation should not directly handle events"
        );
...
mod architecture_integration_tests {
    ...
    fn test_dependency_graph_is_acyclic() {
        ...
        assert!(
            !tree_output.contains("imkitchen-user -> imkitchen-web"),
            "Should not have circular dependency user -> web"
        );
        assert!(
            !tree_output.contains("imkitchen-shared -> imkitchen-user"),
