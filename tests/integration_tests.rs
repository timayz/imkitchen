...
"#,
    ...
    fn run_command(&self, args: &[&str]) -> std::process::Output {
        ...
        cmd.args(args).arg("--config").arg(&self.config_path);
...
async fn test_config_generation_and_validation() {
    ...
    assert!(
        ...
        "Config validation output unexpected"
    );
    ...
    assert!(
        ...
        "Config show output unexpected"
    );
    assert!(
        stdout.contains("Database URL"),
        "Config show missing database URL"
    );
}
...
async fn test_database_operations() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Migrations completed"),
        "Migration up output unexpected"
    );
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("migration(s) applied"),
        "Migration status output unexpected"
    );
}
...
async fn test_health_check() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Database: Connected"),
        "Health check missing database status"
    );
    assert!(
        stdout.contains("Configuration: Valid"),
        "Health check missing config status"
    );
    assert!(
        stdout.contains("System: OK"),
        "Health check missing system status"
    );
}
...
async fn test_error_handling() {
    ...
    assert!(
        output.status.success(),
        "Should use defaults with missing config"
    );
}
...
async fn test_command_line_overrides() {
    ...
    let output = env.run_command(&[
        "--database-url",
        &format!("sqlite:{}", custom_db.to_string_lossy()),
        "migrate",
        "up",
    ]);
    ...
    assert!(
        output.status.success(),
        "Migration with DB override failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(custom_db.exists(), "Custom database file was not created");

    // Test log level override
    let output = env.run_command(&["--log-level", "debug", "health"]);

    assert!(
        output.status.success(),
        "Health check with log override failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_concurrent_operations() {
    ...
    let futures: Vec<_> = (0..5).map(|_| env.run_command_async(&["health"])).collect();
...
async fn test_migration_rollback() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Rollback completed"),
        "Rollback output unexpected"
    );
}
...
async fn test_graceful_shutdown_command() {
    ...
    assert!(
        output.status.success(),
        "Stop command should succeed even with no running server"
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("No running process found"),
...
"#,
    ...
    assert!(
        !output.status.success(),
        "Should fail security validation with short session secret"
    );
    assert!(
