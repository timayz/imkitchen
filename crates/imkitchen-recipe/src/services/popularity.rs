...
impl PopularityService {
    ...
    pub async fn calculate_base_score(&self, recipe_id: Uuid) -> Result<f64, PopularityError> {
        ...
        let rating_score = metrics.rating_average
            * (metrics.rating_count as f64).sqrt()
        ...
        let recency_factor =
            (-days_since_update as f64 / self.config.recency_decay_days as f64).exp();

        let base_score = (view_score + rating_score + bookmark_score) * recency_factor;

        // Normalize to 0-100 scale
        let normalized_score = (base_score * 10.0).clamp(0.0, 100.0);
    ...
    pub async fn calculate_trending_score(
        ...
        let current_metrics = self
            .get_recipe_metrics_for_window(recipe_id, time_window)
            .await?;
        ...
        let previous_metrics = self
            .get_recipe_metrics_for_window(recipe_id, &previous_window)
            .await?;
    ...
    pub async fn get_trending_recipes(
        ...
        for recipe_id in recipe_ids.into_iter().take(limit * 2) {
            // Get more to filter
    ...
    pub async fn analyze_trend_factors(
        ...
        let bookmark_velocity = self
            .calculate_bookmark_velocity(recipe_id, time_window)
            .await?;
    ...
    pub async fn get_popularity_rankings(
        ...
        for recipe_id in recipe_ids {
            let score = match time_window.label.as_str() {
                "24h" => {
                    self.calculate_trending_score(recipe_id, time_window)
                        .await?
                }
    ...
    async fn get_recipes_by_category(
        &self,
        _category: String,
    ) -> Result<Vec<Uuid>, PopularityError> {
    ...
    async fn calculate_bookmark_velocity(
        &self,
        recipe_id: Uuid,
        time_window: &TimeWindow,
    ) -> Result<f64, PopularityError> {
    ...
    async fn get_search_metrics(
        &self,
        recipe_id: Uuid,
        _time_window: &TimeWindow,
    ) -> Result<SearchMetrics, PopularityError> {
        Ok(SearchMetrics {
    ...
    async fn calculate_recent_activity_score(
        &self,
        recipe_id: Uuid,
    ) -> Result<f64, PopularityError> {
        let metrics = self.get_recipe_metrics(recipe_id).await?;
        ...
        Ok(activity_score.clamp(0.0, 100.0))
    }

    async fn save_popularity_metrics(
        &self,
        _metrics: &PopularityMetrics,
