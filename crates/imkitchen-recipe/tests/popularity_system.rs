use imkitchen_recipe::services::popularity::{
    PopularityService, PopularityConfig, TimeWindow, TrendFactor
};
use uuid::Uuid;
use chrono::Duration;
...
async fn test_calculate_trending_score() {
    ...
    let trending_score = service.calculate_trending_score(recipe_id, &time_window).await.unwrap();
...
async fn test_get_trending_recipes() {
    ...
    let trending_recipes = service.get_trending_recipes(&time_window, 10).await.unwrap();
...
async fn test_analyze_trend_factors() {
    ...
    let factors = service.analyze_trend_factors(recipe_id, &time_window).await.unwrap();
    ...
    for factor in &factors {
        match factor {
            TrendFactor::ViewSpike { multiplier, timeframe } => {
                assert!(*multiplier > 1.0);
                assert!(!timeframe.is_empty());
            },
            TrendFactor::RatingBoost { average_rating, recent_ratings } => {
            ...
            TrendFactor::NewRecipe { days_old, novelty_boost } => {
                assert!(*days_old >= 0);
                assert!(*novelty_boost >= 0.0 && *novelty_boost <= 1.0);
            },
            TrendFactor::SearchPopularity { query_frequency, search_rank } => {
                assert!(*query_frequency > 0);
                assert!(*search_rank > 0);
            },
            TrendFactor::SeasonalRelevance { relevance_score, context } => {
...
async fn test_get_popularity_rankings() {
    ...
    let rankings = service.get_popularity_rankings(None, &time_window, 15).await.unwrap();
...
async fn test_get_popularity_rankings_with_category() {
    ...
    let rankings = service.get_popularity_rankings(Some(category), &time_window, 10).await.unwrap();
...
async fn test_different_time_windows() {
    ...
    let score_24h = service.calculate_trending_score(recipe_id, &TimeWindow::last_24_hours()).await.unwrap();
    let score_7d = service.calculate_trending_score(recipe_id, &TimeWindow::last_week()).await.unwrap();
    let score_30d = service.calculate_trending_score(recipe_id, &TimeWindow::last_month()).await.unwrap();
...
async fn test_trending_recipes_with_different_windows() {
    ...
    let trending_24h = service.get_trending_recipes(&TimeWindow::last_24_hours(), 5).await.unwrap();
    let trending_7d = service.get_trending_recipes(&TimeWindow::last_week(), 5).await.unwrap();
    let trending_30d = service.get_trending_recipes(&TimeWindow::last_month(), 5).await.unwrap();
...
async fn test_trend_factors_variety() {
    ...
    let has_view_spike = factors.iter().any(|f| matches!(f, TrendFactor::ViewSpike { .. }));
    let has_rating_boost = factors.iter().any(|f| matches!(f, TrendFactor::RatingBoost { .. }));
    let has_new_recipe = factors.iter().any(|f| matches!(f, TrendFactor::NewRecipe { .. }));
    let has_search_popularity = factors.iter().any(|f| matches!(f, TrendFactor::SearchPopularity { .. }));
...
async fn test_empty_results_handling() {
    ...
    let trending_recipes = service.get_trending_recipes(&TimeWindow::last_24_hours(), 0).await.unwrap();
    assert!(trending_recipes.is_empty());
    
    let rankings = service.get_popularity_rankings(None, &TimeWindow::last_24_hours(), 0).await.unwrap();
    assert!(rankings.is_empty());
}
...
async fn test_large_limit_handling() {
    ...
    let trending_recipes = service.get_trending_recipes(&TimeWindow::last_24_hours(), 1000).await.unwrap();
    assert!(trending_recipes.len() <= 1000); // Should not exceed reasonable bounds
    
    let rankings = service.get_popularity_rankings(None, &TimeWindow::last_24_hours(), 1000).await.unwrap();
...
async fn test_popularity_score_bounds() {
    ...
    let trending_score = service.calculate_trending_score(recipe_id, &TimeWindow::last_24_hours()).await.unwrap();
