use imkitchen_recipe::domain::{Difficulty, RecipeCategory};
...
async fn test_generate_suggestions_with_recipes() {
    let service = RecipeSearchService::new();
    let suggestions = service
        .generate_suggestions("pasta", None, 10)
        .await
        .unwrap();
    ...
    assert!(suggestions
        .iter()
        .any(|s| s.text.to_lowercase().contains("pasta")));
}
...
async fn test_generate_suggestions_with_tags() {
    let service = RecipeSearchService::new();
    let suggestions = service
        .generate_suggestions("quick", None, 10)
        .await
        .unwrap();
...
async fn test_typo_correction_suggestions() {
    let service = RecipeSearchService::new();
    let suggestions = service
        .generate_suggestions("chickn", None, 10)
        .await
        .unwrap();
...
async fn test_suggestions_scoring() {
    let service = RecipeSearchService::new();
    let suggestions = service
        .generate_suggestions("chicken", None, 10)
        .await
        .unwrap();
...
async fn test_user_preference_weighting() {
    ...
    let suggestions = service
        .generate_suggestions("chicken", Some(&preferences), 10)
        .await
        .unwrap();
...
async fn test_user_preference_updates() {
    ...
    let result = service
        .update_user_preferences(user_id, "chicken recipe", recipe_id)
        .await;
    assert!(result.is_ok());
}
...
async fn test_very_long_query_handling() {
    ...
    let suggestions = service
        .generate_suggestions(&long_query, None, 10)
        .await
        .unwrap();
...
async fn test_case_insensitive_search() {
    ...
    let lower_suggestions = service
        .generate_suggestions("chicken", None, 10)
        .await
        .unwrap();
    let upper_suggestions = service
        .generate_suggestions("CHICKEN", None, 10)
        .await
        .unwrap();
    let mixed_suggestions = service
        .generate_suggestions("ChIcKeN", None, 10)
        .await
        .unwrap();
...
async fn test_similarity_reasons_structure() {
    ...
    for similarity in &similarities {
        ...
        for reason in &similarity.similarity_reasons {
            match reason {
                imkitchen_recipe::services::search::SimilarityReason::IngredientOverlap {
                    common_ingredients,
                    overlap_score,
                } => {
                ...
                imkitchen_recipe::services::search::SimilarityReason::CookingTechnique {
                    common_techniques,
                    technique_score,
                } => {
