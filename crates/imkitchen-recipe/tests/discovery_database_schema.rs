use imkitchen_recipe::services::discovery_data::{
    DiscoveryDataService, PreferenceType, SimilarityType, EventType, SourceContext, QueryType,
    RecipeDiscoveryEvent, DiscoveryQueryAnalytics, UserDiscoveryPreference, RecipeSimilarityCache
};
use uuid::Uuid;
use chrono::Utc;
...
async fn test_update_user_preference() {
    ...
    let result = service.update_user_preference(
        user_id,
...
async fn test_preference_type_conversion() {
    ...
    assert!(matches!(PreferenceType::from_str("category"), Some(PreferenceType::Category)));
    assert!(matches!(PreferenceType::from_str("difficulty"), Some(PreferenceType::Difficulty)));
...
async fn test_get_popular_recipes() {
    ...
    for i in 1..popular_recipes.len() {
        assert!(popular_recipes[i-1].base_popularity_score >= popular_recipes[i].base_popularity_score);
    }
}
...
async fn test_get_trending_recipes() {
    ...
    for i in 1..trending_recipes.len() {
        assert!(trending_recipes[i-1].trending_score_24h >= trending_recipes[i].trending_score_24h);
    }
}
...
async fn test_get_analytics_summary() {
    ...
    for day_summary in &summary {
        ...
        assert!(day_summary.avg_click_through_rate >= 0.0 && day_summary.avg_click_through_rate <= 1.0);
...
async fn test_update_feature_config() {
    ...
    let result = service.update_feature_config(
        "search_max_results",
        "100",
        Some("admin_user"),
    ).await;
...
async fn test_search_recipes_fts() {
    ...
    let results = service.search_recipes_fts("chicken pasta", 10, 0).await.unwrap();
...
async fn test_complex_user_preferences_workflow() {
    ...
    let update_results = vec![
        service.update_user_preference(user_id, PreferenceType::Category, "mediterranean", 0.9).await,
        service.update_user_preference(user_id, PreferenceType::Difficulty, "intermediate", 0.7).await,
        service.update_user_preference(user_id, PreferenceType::PrepTime, "quick", 0.8).await,
    ];
