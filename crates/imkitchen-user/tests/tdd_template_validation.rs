...
mod template_validation_tests {
    ...
    fn test_profile_form_validation_family_size() {
        ...
        for size in valid_sizes {
            ...
            assert!(result.is_ok(), "Family size {} should be valid", size);
        }
        ...
        for size in invalid_sizes {
            let result = FamilySize::new(size);
            assert!(result.is_err(), "Family size {} should be invalid", size);
        }
    }
    ...
    fn test_dietary_restrictions_validation() {
        ...
        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&single_restriction);
        assert!(result.is_ok(), "Single dietary restriction should be valid");
        ...
        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&compatible_restrictions);
        assert!(
            result.is_ok(),
        ...
        let conflicting_restrictions =
            vec![DietaryRestriction::Vegan, DietaryRestriction::Vegetarian];
        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&conflicting_restrictions);
        assert!(
            result.is_err(),
        ...
        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&too_many_restrictions);
        assert!(
            result.is_err(),
    ...
    fn test_profile_completeness_validation() {
        ...
        assert!(
            completeness < 100.0,
            "Incomplete profile should have less than 100% completeness: {}%",
            completeness
        );
        assert!(
            completeness >= 50.0,
            "Profile with family size and skill should have at least 50% completeness: {}%",
            completeness
        );
    ...
    fn test_cooking_time_validation_ranges() {
        ...
        for (weekday, weekend, _should_be_valid) in test_cases {
            ...
            assert!(
                ...
                "Cooking efficiency should be between 0 and 100: {}",
                efficiency
            );
        }
    }
...
mod workflow_validation_tests {
    ...
    fn test_complete_profile_update_workflow() {
        ...
        let compatibility = ProfileService::validate_dietary_restrictions_compatibility(
            &dietary_command.new_restrictions,
        );
        assert!(
            compatibility.is_ok(),
    ...
    fn test_error_handling_workflow() {
        ...
        let compatibility = ProfileService::validate_dietary_restrictions_compatibility(
            &conflicting_dietary.new_restrictions,
        );
        assert!(
            compatibility.is_err(),
        ...
        assert!(
            errors
                .iter()
                .any(|err| err.contains("Vegan diet already includes vegetarian")),
...
mod integration_validation_tests {
    ...
    fn test_end_to_end_profile_creation() {
        ...
        assert!(
            family_update.validate_command().is_ok(),
        ...
        assert!(
            skill_update.validate_command().is_ok(),
        ...
        // Verify final state
        let final_completeness =
            ProfileService::calculate_profile_completeness(&final_user.profile);
        assert_eq!(
            final_completeness, 100.0,
    ...
    fn test_validation_error_handling() {
        ...
        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&conflicting_restrictions);
        assert!(result.is_err(), "Should detect vegan-vegetarian conflict");

        // 2. Test keto-low carb conflict
        let keto_lowcarb_conflict = vec![DietaryRestriction::Keto, DietaryRestriction::LowCarb];

        let result =
            ProfileService::validate_dietary_restrictions_compatibility(&keto_lowcarb_conflict);
        assert!(result.is_err(), "Should detect keto-low carb conflict");
        ...
        assert!(result.is_err(), "Should detect too many restrictions");

        let errors = result.unwrap_err();
        assert!(
            errors
                .iter()
                .any(|err| err.contains("Too many dietary restrictions")),
    ...
    fn test_recommendation_integration() {
        ...
        assert_eq!(
            weekday_criteria.max_cooking_time, 45,
        ...
        assert_eq!(
            weekend_criteria.max_cooking_time, 90,
            "Weekend max cooking time should match profile"
        );
        assert!(
            weekend_criteria
                .complexity_levels
                .contains(&"Complex".to_string()),
        ...
        assert!(
            meal_recommendations.weekly_meal_count > 0,
            "Should recommend weekly meals"
        );
        assert!(
            !meal_recommendations.prep_day_suggestions.is_empty(),
            "Should suggest prep days"
        );
        assert_eq!(
            meal_recommendations.cooking_schedule.len(),
            7,
            "Should have schedule for all 7 days"
        );

        // Test skill progression
        let skill_progression =
            RecommendationService::generate_skill_progression_recommendations(&profile);
        assert_eq!(
            skill_progression.current_level,
            SkillLevel::Intermediate,
            "Should correctly identify current skill level"
        );
        assert!(
            !skill_progression.next_level_requirements.is_empty(),
...
mod performance_validation_tests {
    ...
    fn test_validation_performance() {
        ...
        assert!(
            avg_time_per_iteration < 1000.0, // Less than 1ms per iteration
            "Average validation time should be under 1ms, got {:.2}μs",
        ...
        println!(
            "Validation performance: {:.2}μs per iteration ({} iterations in {:?})",
