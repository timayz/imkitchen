...
use crate::{
    ...
    events::{UserAccountDeleted, DeletionReason},
...
impl AccountDeletionHandler {
    ...
    pub async fn handle_delete_account(
        ...
        self.event_store.store_event(&deletion_event).await
            .map_err(|e| UserError::DatabaseError(format!("Failed to store deletion event: {}", e)))?;
        
        // Mark user account as deleted in database
        self.mark_user_deleted(&mut tx, command.user_id, &deletion_event).await?;
        
        // Initiate cascading deletions across bounded contexts
        let cascading_operations = self.initiate_cascading_deletions(&mut tx, command.user_id).await?;
        
        // Create audit trail entry
        self.create_audit_trail_entry(&mut tx, &deletion_event, command.admin_user_id).await?;
        
        // Commit transaction
        tx.commit().await
            .map_err(|e| UserError::DatabaseError(format!("Failed to commit deletion transaction: {}", e)))?;
        ...
        Ok(AccountDeletionResponse {
            ...
            message: format!("Account deleted successfully. Data purge scheduled for {}", 
                deletion_event.data_purge_scheduled_at.format("%Y-%m-%d %H:%M:%S UTC")),
        })
    }
    ...
    pub async fn handle_purge_user_data(
        ...
        self.create_purge_audit_entry(&mut tx, command.user_id, command.admin_authorization).await?;
        
        // Commit transaction
        tx.commit().await
            .map_err(|e| UserError::DatabaseError(format!("Failed to commit purge transaction: {}", e)))?;
    ...
    async fn load_user(&self, user_id: Uuid) -> Result<User, UserError> {
        ...
        use imkitchen_shared::{FamilySize, SkillLevel, DietaryRestriction, Email};
    ...
    async fn mark_user_deleted(
        ...
        .bind(deletion_event.data_purge_scheduled_at.format("%Y-%m-%d %H:%M:%S%.3fZ").to_string())
        .bind(user_id.to_string())
        .execute(&mut **tx)
        .await
    ...
    async fn initiate_cascading_deletions(
        ...
        sqlx::query("UPDATE user_preferences SET deleted_at = ? WHERE user_id = ?")
            ...
            .execute(&mut **tx)
            .await
            .map_err(|e| UserError::DatabaseError(format!("Failed to delete user preferences: {}", e)))?;
    ...
    async fn purge_all_user_data(
        ...
        sqlx::query("DELETE FROM user_profiles WHERE id = ?")
            ...
            .execute(&mut **tx)
            .await
            .map_err(|e| UserError::DatabaseError(format!("Failed to purge user profile: {}", e)))?;
        ...
        sqlx::query("DELETE FROM user_preferences WHERE user_id = ?")
            ...
            .execute(&mut **tx)
            .await
            .map_err(|e| UserError::DatabaseError(format!("Failed to purge user preferences: {}", e)))?;
    ...
    async fn create_purge_audit_entry(
        ...
        .execute(&mut **tx)
        .await
        .map_err(|e| UserError::DatabaseError(format!("Failed to create purge audit entry: {}", e)))?;
...
mod tests {
    ...
    fn test_purge_user_data_command_validation() {
        ...
        let valid_command = PurgeUserDataCommand::new(
            Uuid::new_v4(),
            past_time,
            Uuid::new_v4(),
        );
        ...
        let invalid_command = PurgeUserDataCommand::new(
            Uuid::new_v4(),
            future_time,
            Uuid::new_v4(),
        );
    ...
    fn test_user_account_deleted_event_creation() {
        ...
        // Test GDPR deletion
        let gdpr_deletion = UserAccountDeleted::gdpr_request(
            user_id,
            email,
            account_created_at,
        );
