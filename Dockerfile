...
COPY --from=builder --chown=imkitchen /app/crates/imkitchen-web/static crates/imkitchen-web/static
COPY --from=builder --chown=imkitchen /my_dir /var/data
COPY --from=builder --chown=imkitchen /my_dir /tmp

USER imkitchen:imkitchen

EXPOSE 3000

ENTRYPOINT ["imkitchen", "--config", "/etc/imkitchen/config.toml"]
CMD ["web", "start"]
