...
pub fn routes() -> axum::Router<imkitchen_web_shared::AppState> {
    ...
    axum::Router::new()
        .route("/upgrade", get(routes::upgrade::page).post(routes::upgrade::action))
        .route("/upgrade/order-summary", get(routes::upgrade::order_summary))
        ...
        .route("/contact", get(routes::contact::page).post(routes::contact::action))
        .route("/register", get(routes::register::page).post(routes::register::action))
        .route("/login", get(routes::login::page).post(routes::login::action))
        .route("/reset-password", get(routes::reset_password::page).post(routes::reset_password::action))
        .route("/reset-password/new/{id}", get(routes::reset_password::new_page).post(routes::reset_password::new_action))
