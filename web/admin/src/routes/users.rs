...
use imkitchen_identity::types::user::{Role, State as UserState};
...
pub async fn page(
    ...
    let users = imkitchen_web_shared::try_page_response!(
        app.identity.filter_admin(FilterQuery {
            state,
            sort_by,
            role,
            args: args.limit(20),
...
pub async fn suspend(
    ...
    imkitchen_web_shared::try_response!(app.identity.suspend(&id, &admin.id), template);
...
pub async fn activate(
    ...
    imkitchen_web_shared::try_response!(app.identity.activate(&id, &admin.id), template);
