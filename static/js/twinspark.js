...
(function(window, document, tsname) {
    ...
    return {
      promise: new Promise(function(resolve, reject) {
        ...
        xhr.onreadystatechange = function() {
          ...
          var headers = {
            'ts-title': xhr.getResponseHeader('ts-title'),
  ...
  function doReqBatch(reqs) {
    ...
    return Promise
      .all(reqs.map(function(req) {
        ...
        var detail = {
          req: req,
          originalTarget: ((req.event.detail && req.event.detail.originalTarget) ||
            req.event.target)
        };
  ...
  function parseActionSpec(s) {
    ...
    function consumeCommand() {
      ...
      action.push({
        src: s.slice(command_start, i).trim(),
        name: command[0],
    ...
    function consumeAction() {
      ...
      actions.push({
        src: s.slice(action_start, i).trim(),
        commands: action
      });
      action = [];
      action_start = i + 1;
  ...
  function makeTriggerListener(t) {
    ...
    var spec = {
      changed: t.args.indexOf('changed') != -1,
